...
use crate::platform_impl::platform::ffi::{NO, id, nil};
use objc2::{
    ...
    match self {
      ...
      Cursor::Undocumented(cursor_name) => {
        ...
        let sel = if msg_send![class, respondsToSelector: sel] {
          ...
          warn!("Cursor `{cursor_name}` appears to be invalid");
          sel!(arrowCursor)
        };
        msg_send![class, performSelector: sel]
      }
      Cursor::WebKit(cursor_name) => unsafe { load_webkit_cursor(cursor_name) },
    }
  }
}
...
pub unsafe fn load_webkit_cursor(cursor_name: &str) -> id {
  ...
  #[allow(deprecated)]
  let info =
    unsafe { NSDictionary::<AnyObject, AnyObject>::dictionaryWithContentsOfFile(&info_path) }
      .unwrap();
