...
unsafe fn create_view(
  ...
  ns_view.map(|ns_view| {
    ...
    if let Some(position) = pl_attribs.traffic_light_inset {
      let state_ptr: *mut c_void = unsafe { *(**ns_view).get_ivar("taoState") };
      let state = unsafe { &mut *(state_ptr as *mut ViewState) };
...
pub struct SharedState {
  ...
  #[allow(dead_code)] // Used for fullscreen mode restoration on macOS
...
pub struct UnownedWindow {
  ...
  transparent: bool,
  #[allow(dead_code)] // Cached inner rectangle size for window operations
