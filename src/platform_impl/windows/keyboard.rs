...
use crate::{
  ...
  platform_impl::{
    ...
    platform::{
      ...
      keyboard_layout::{LAYOUT_CACHE, Layout, LayoutCache, WindowsModifiers, get_or_insert_str},
    },
  },
};
    ...
    match msg_kind {
      ...
      win32wm::WM_CHAR | win32wm::WM_SYSCHAR => {
        if self.event_info.is_none() {
          trace!(
            "Received a CHAR message but no `event_info` was available. The message is probably IME, returning."
          );
          return vec![];
        }
        *result = ProcResult::Value(LRESULT(0));
